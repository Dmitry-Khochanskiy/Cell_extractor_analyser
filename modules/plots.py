...
def pair_grid(df, columns_for_analysis, savepath):
...
# pair grid
    ...
    g.savefig(f"{savepath}/pair_grid.png")
    plt.show()


def heat_map(df, columns_for_analysis, savepath):
# heat map on normalizied image
    df_norm = df / df.sum()
    sns.set(rc={'figure.figsize':(11.7,8.27)})
    ax = sns.heatmap(df_norm[columns_for_analysis])
    figure = ax.get_figure()
    figure.savefig(f"{savepath}/heat_map.png")
    plt.show()

def correlation_heat_map(df, columns_for_analysis, savepath):
...
# Correlation heatmap
    ...
    figure.savefig(f"{savepath}/cor_heat_map.png")
    plt.show()
